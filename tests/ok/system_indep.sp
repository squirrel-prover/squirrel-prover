...
global lemma  [set:B/left; equiv:B/left, B/right] _ : equiv(frame@(tt@init)).
Proof. 
  checkfail have ? := gfooB' (tt@init) exn Failure. 
Abort.

(*------------------------------------------------------------------*)
(* the system-independence check fails if there is more than one single system. *)
global axiom [set:B/right; equiv:B/left, B/left] gfooB'' (t:timestamp): equiv(frame@t).
global lemma  [set:B/right; equiv:B/left, B/left] _ : equiv(frame@(tt@init)).
Proof. 
  checkfail have ? := gfooB'' (tt@init) exn Failure. 
Abort.
