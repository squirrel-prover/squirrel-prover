...
global goal Gi (t : _[const]) : [forall i, m i <> t].
Proof. 
  intro i => Meq. 
  fresh Meq. 
Qed.
...
goal _ (t : _[const,glob]) : (fun i => m i <> t) = ptrue.
Proof.
  rewrite Gi. 
  rewrite /ptrue //.
Qed.
...
(* It is not true that `forall i, n i <> t` is almost always true,
   ...
   the `n i` and `t`) *)
global goal _ (t : _[const]) : [forall i, n i <> t].
Proof. 
  intro i => Meq. 
...
global goal GI (t : _[const]) : Forall (i : _[const]), [n i <> t].
Proof.  
  intro i => Meq. 
  fresh Meq.
Qed.
...
goal _ (t : _[const,glob]) : (fun i => n i <> t) = ptrue.
Proof.
