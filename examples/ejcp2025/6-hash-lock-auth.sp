...
Proof.
  ...
  (* Note that it is necessary that `R1(j)` happens, or the macro expansion 
     will fail. Test this by removing the happens hypothesis. *)
 
  (* If the timestamp `t` occurs, the macro `m@t` can be expanded into 
     its definition using the tactic `rewrite /m`. 
...
(* In the reader process, we see that the action `R1(j)` always 
   ...
   This is expressed by an axiom, generated when the system is declared. *)
lemma macro_1 (j : index) : 
  happens(R1(j)) => happens(R(j)).
Proof.
  intro Hap.
  (* Here smt (if functional) succeeds but not auto does not:
     auto knows about the axioms governing happens and the order on
     timestamps, but it does not know system-specific axioms.
     To help auto, we need to manually use the relevant lemma.
     Let's try to find the relevant axiom by searching a pattern
     in all available axioms and lemmas. *)
  search (happens (R _)). (* Nothing useful here... *)
  search (R _ < R1 _).    (* Aha! *)
  have H : R j < R1 j.
  + apply depends_R_R1 j Hap.
  + auto.
Qed.

(* Here macro unfolding takes the dependency into account, so the whole
   lemma is proved automatically. *)
...
(* First, we prove that if the session `k` of the tag `i` (which is `T(i,k)`)
