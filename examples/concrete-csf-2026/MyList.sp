...
let rec length (l : list) with
| Emp -> zn
| Cons _ l -> sn (length l).
Proof. intro > <-; discriminate. Qed.

let rec in_list (a : lval) (l : list) with
| Emp -> false
| Cons b l -> (a = b) || (in_list a l).
Proof. intro > <- _; discriminate. Qed.

let rec get (n : nat) (l : list ) : o_lval with
| Emp -> l_none
| Cons a l ->
...
(* basic lemmas *)
exact lemma [any] length_empty (l : list) : length l = zn => l = Emp.
Proof.
case l; 1:  auto.
...
exact lemma [any] empty_length (l : list) : l = Emp => length l = zn.
Proof.
case l; 1:  auto.
+ intro h l0; intro H; discriminate H.
+ auto. 
...
exact lemma [any] get_in_cons (h : lval) (t : list) (n : nat) :
    get n t = get (sn n) (Cons h t).
Proof.
generalize h n.
induction t.
...
exact lemma [any] get_in_range (l : list) :
    forall(n : nat),
        ltn (n,(predn (length l))) && l <> Emp => get n l <> l_none.
Proof.
