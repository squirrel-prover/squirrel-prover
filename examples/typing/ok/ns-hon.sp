...
process Init(i,j,k:index)=
  I1: out(c, <id(i),<id(j),Na(i,j,k)>>);
  I2: in(c,x);  
      let mI = dec(x,Ks(i)) in
      let kI = fst(snd(snd(mI))) in
      if mI <> fail && fst(mI) = id(j) && fst(snd(mI)) = Na(i,j,k)
      then out(c, snd(snd(snd(mI)))). 
...
process Resp(i,j,k:index)=
  R: in(c, x);
     ...
     if mR <> fail && fst(mR) = resp  && snd(snd(mR)) = id(i) 
     then out(c,empty).
...
lemma key_secrecy_resp : forall (tau:timestamp), forall (i,j,k:index),
   ...
   att(frame@tau) <> 
   if cond@R(i,j,k) then kR@R(i,j,k) else Kfresh.
Proof.
  intro *.
  expandall.
  by typing Meq.
