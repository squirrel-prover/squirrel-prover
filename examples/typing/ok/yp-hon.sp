...
process Init(i,j,k:index)=
  ...
  I2: in(c,x);
      let mI = dec(fst(snd(x)),Ks(i)) in
      let kI = fst(snd(snd(mI))) in
      if mI <> fail 
      && fst(mI) = tag2
      && fst(snd(mI)) = id(j)
      && snd(snd(snd(mI))) = Na(i,j,k)
      then out(c, snd(snd(x))).

process Server(i,j,k:index)=
  S: in(c, x);
     let mS = dec(snd(snd(x)),Ks(j)) in
     if fst(x) = id(j) 
     && mS <> fail
     && fst(mS) = tag1
     && fst(snd(mS)) = id(i)
     then out(c, <fst(snd(x)), 
                 <enc(<tag2,<id(j),<K(i,j,k),snd(snd(snd(mS)))>>>, Rsa(i,j,k),Ks(i)),
...
process Resp(i,j,k:index)=
  ...
  R2: in(c,y);
      ...
      let kR = fst(snd(snd(snd(mR)))) in
      if mR <> fail 
      && fst(mR) = tag3
      && fst(snd(mR)) = id(i)
      && fst(snd(snd(mR))) = id(j)
      && snd(snd(snd(mR))) = Nb(i,j,k)
      then out(c,empty).
