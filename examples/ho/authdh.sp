...
lemma [default] A_agreement (A,B,i:index):
   ...
   (cond@A2(A,B,i) <=>
    exists (j:index),
      B1(A,B,j) < A2(A,B,i) && 
      gkA@A2(A,B,i) = gen^(x(A,B,i) ** y (A,B,j)) &&
      fst (input@A2(A,B,i)) = fst (output@B1(A,B,j)) &&
      input@B1(A, B, j) = ofG (gen ^ xA@A2(A, B, i)) &&
...
Proof.
  ...
  - intro [j [H1 [H2 H3 H4 H5]]]. 
    rewrite /cond /sA. 
    have ->: gy@A2(A, B, i) = ofG (gen ^ y(A, B, j)) by auto.
    have ->: ofG (gen ^ xA@A1(A, B, i)) = ofG (gen ^ x(A, B, i)) by auto. 
    auto. 
Qed.
...
lemma [default] B_agreement (A,B,i:index):
   ...
   cond@B2(A,B,i) =>
   exists (j:index),
     A2(A,B,j) < B2(A,B,i) && gkB@B2(A,B,i) = gen^(x(A,B,j) ** y (A,B,i)).
Proof.
  intro Hhap HB Hc.
  rewrite /cond in Hc.
...
global lemma [set:default/left; equiv:default/left, default/left]
  ...
  equiv(
    frame@t0, 
    if cond@t then diff(kA@t, nfresh)).
Proof.
