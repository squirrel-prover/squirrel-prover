...
exact axiom [any] encap_public_spec : forall x y, encap_public x y = (encap x y # 2).

abstract encap_shared : kem_randomness -> message -> kem_shared.
exact axiom [any] encap_shared_spec : forall x y, encap_shared x y = (encap x y # 1).

exact lemma [any] decap_encap_public :
...
tactic xsmt = (try project); expandall; smt ~no_macros.
...
Proof.
  intro Hap Hcond.
  project.
  + rewrite R_WauthA_eq in Hcond; xsmt.
  + rewrite R_WauthA_eq in Hcond; xsmt.
  + rewrite I_WauthA_eq in Hcond; xsmt.
  + rewrite I_WauthA_eq in Hcond; xsmt.
Qed.
...
Proof.
  intro _ Hcond.
  assert happens(B1) as _ by apply cond_A2_happens_B.
  have _ := cond_A2_input_B. xsmt.
Qed.
...
Proof.
  intro Ha Hc.
  project.
  + rewrite R_WauthA_eq in Hc; xsmt.
  + rewrite R_WauthA_eq in Hc; xsmt.
  + rewrite I_WauthA_eq in Hc; xsmt.
  + rewrite I_WauthA_eq in Hc; xsmt.
Qed.
...
Proof.
  ...
  * (** First implication *)
    ...
    euf EqSignB.
    intro [H1 H2].
    assert (A2 < tau) by smt.
    assert exec@A2 as Hexec by apply exec_le (pred tau) A2.
    assert cond@A2 as HcondA2 by xsmt.
...
Proof.
  ...
  * (** First implication *)
    ...
    euf EqSignB.
    intro [H1 H2].
    assert (A2 < tau) by xsmt.
    assert exec@A2 as _ by apply exec_le (pred tau) A2.
    expand exec@A2.
...
Proof.
  ...
  + (** A2 *)
    ...
    rewrite !cond_A2_input_A1 /= in 6; [1,2,3,4,5,6: xsmt].
  ...
  + (** A3 *)
    ...
    rewrite /input. fa 2. fa(qatt _); 1: xsmt.
    apply IH.
  ...
  + (** B1 *)
    ...
    deduce.
    rewrite /kB.
    rewrite (eq_eq (encap_shared r (input@B)) (encap_shared r (kem_pub eskA))); 1: xsmt.
    ...
    assert(diff(
           ...
           diff(encap_shared r (kem_pub eskA), kBh) else encap_shared r' (input@B)) by xsmt.
  ...
  + (** B3 *)
    ...
    fa (_,_,_), !<_,_>.
    rewrite /exec /cond. 
    rewrite WauthB_eq_RI in 5; 1,2: constraints.
    deduce 5.
    rewrite /input. fa 2. fa(qatt _); 1: xsmt.
    apply IH.
Qed.
...
Proof.
  ...
  + (** A *)
    ...
    fa (_,_,_); fa !<_,_>. fa 6. fa 2. fa(qatt _); 1: constraints. apply IH.
  ...
  + (** A2 *)
    ...
    rewrite cond_A2_input_A1 in 1; 1,2: auto.
    rewrite cond_A2_input_A2 in 1; 1,2: auto.
  ...
  + (** B2 *)
    ...
    rewrite exec_B2_input_B1 in 1 => //.
    fa (_,_,_), !<_,_>.
    assert B1 < B2 by xsmt.
    assert B  < B1 by xsmt.
    rewrite exec_B2_input_B //= in 6.

    prf 6; 1: smt. (* TODO here expandall takes forever *)
...
Proof.
  ...
  + (** A *)
    rewrite /frame /state /transcript /output /exec /cond; fa (_,_,_), !<_,_>.
    rewrite /epkA.
    rewrite /input. fa 2. fa(qatt _); 1: constraints.
    apply IH.

  + (** A1 *)
    rewrite /frame /state /transcript /output; fa (_,_,_), !<_,_>.
    rewrite /input. fa 2. fa(qatt _); 1: smt.
    apply IH.

  + (** A2 *)
    rewrite /frame /state /transcript /output /xrB /kmacA /epkA; fa (_,_,_), !<_,_>.
    rewrite !cond_A2_input_A1 /= in 6; [1,2,3,4,5,6: xsmt].
    rewrite !cond_A2_input_A2 /= in 6; [1,2: xsmt].
    fa (if _ then _), !<_,_>, (sign _), (mac _), !<_,_>.
  ...
  + (** B *)
    ...
    rewrite !(eq_eq (encap_public r (input@B)) (encap_public r (kem_pub eskA)));
      1: smt.
    rewrite /input. fa 6. fa 6. fa 6.  fa 8. fa 2. fa(qatt _); 1: smt.
    apply IH.
  ...
  + (** B1 *)
    ...
    rewrite (eq_eq (encap_shared r (input@B)) (encap_shared r (kem_pub eskA))); 1: smt.
    deduce 3.
    fa 1. 
    fa (qatt _); 1: smt.
    apply IH.
  ...
  + (** B2 *)
    ...
    rewrite /input /state. fa 2. fa(qatt _); 1: smt.
    apply IH.
