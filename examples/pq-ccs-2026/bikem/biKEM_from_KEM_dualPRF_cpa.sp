(**
  ...
  In this file, we prove that the construction of a biKEM is CPA-secure
  as soon as at least one of its constituent KEM is CPA-secure.  This is the dualPRF 
  construction from
  ...
  Contrary to eprint.iacr.org/2018/903.pdf we do the proof without ROM
  nor QROM, and we only establish CPA-robustness
...
process P_encap_middle1 =
  out(c_encap,
      <biencap_public (r1,r2) (bikem_pub (sk1,sk2)),
             h(biencap_public (r1,r2) (bikem_pub (sk1,sk2)), dualprf(rand1,encap2_shared r2 (kem2_pub sk2)))>).
...
Proof.
  ...
  + rewrite /frame /transcript /exec /cond /output /state /=. 
    ...
    use ax_dual_prf with (rand1,encap2_shared r2 (kem2_pub sk2)). fa (_,_,_), !<_,_>.
    rewrite H in 5. simpl. 
    fa (if _ then _ else _). fa <_,_>. fa 5. rewrite /input. fa 1. fa(qatt _). {auto. }
    prf 4.  fresh 4 => //. 
    apply IH.
Qed.
...
Proof.
  ...
  + rewrite /frame /transcript /exec /cond /output. 
    rewrite  !biencap_public_spec !bikem_pub_spec /=. fa (_,_,_), !<_,_>. 
    fa (if _ then _ else _). fa !<_,_>. prf 5 => //. fresh 5 => //. 
