...
game KEM2_CCA_SINGLE = {
  ...
  oracle o_decap (c:message) = {
   return if c <> (encap2_public r (kem2_pub skey)) then decap2 c skey
  }
}.
...
(** Idealization for the biKEM shared data, and for the KEM2 shared data *)
name rand: message.
name rand1: message.
name rand': message.
...
lemma [any] decap2_rewrite ['a] (x:message,f:message->'a) :
  f (decap2 x sk2) =
  if x <> encap2_public r2 (kem2_pub sk2) then
    f (decap2 x sk2)
  else
    f (encap2_shared r2 (kem2_pub sk2)).
Proof.
  case (x <> encap2_public r2 (kem2_pub sk2)).
  + auto.
  + by intro ->.
Qed.
...
on the right we idealized the shared kem2 with rand1 *)
...
h(
make_biencap(encap1_public r1 (kem1_pub sk1), encap2_public r2 (kem2_pub sk2)),
dualprf(hext(const0, encap1_shared r1 (kem1_pub sk1)), diff(encap2_shared r2 (kem2_pub sk2),rand1)))>).
...
process P_decap_game01 = 
 in(c_decap, x);
   ...
   then
       out(c_decap, h(make_biencap(extract_kem1 x, extract_kem2 x), 
                      dualprf(hext(const0,decap1 (extract_kem1 x) sk1), 
                          if (extract_kem2 x) <> encap2_public r2 (kem2_pub sk2) 
                                 then decap2 (extract_kem2 x) sk2 
                                 else diff(encap2_shared r2 (kem2_pub sk2),rand1)))).
...
Proof.
 ...
 + rewrite /frame /transcript /exec /output.  
   rewrite  bikem_pub_spec. simpl. 
   fa(_,_,_); fa !<_,_>. rewrite /input /state. fa 1. fa(qatt _). {auto. }.  
   apply IH.
 + rewrite /frame /transcript /exec /output. 
   rewrite  biencap_shared_spec  biencap_public_spec bikem_pub_spec.  simpl. 
   fa(_,_,_); fa !<_,_>. rewrite /input /state. fa 1. fa(qatt _). {auto. }.  
   apply IH. 
 + rewrite /frame /transcript /exec /output.   
   use decap2_rewrite with (extract_kem2 (input@Decap(i))), (fun x:message => x). simpl. 
   fa (_,_,_), !<_,_>. rewrite H in 5. fa 4.  
   rewrite /cond. fa 5. rewrite  biencap_public_spec bikem_pub_spec. simpl. 
...
Proof.
  intro Hap.   crypto KEM2_CCA_SINGLE => //.
Qed.


(** GAME 12: On the right, we now idealized the dualprf(hext(const0,shared_kem1), rand1) by rand' *)
...
process P_encap_game12 = 
  out(c_encap,
     ...
     h(make_biencap(encap1_public r1 (kem1_pub sk1), encap2_public r2 (kem2_pub sk2)),
       diff(dualprf2(hext(const0,encap1_shared r1 (kem1_pub sk1)),rand1), rand'))>).
...
process P_decap_game12 = 
 in(c_decap, x);
    ...
    then
       out(c_decap, h(make_biencap(extract_kem1 x, extract_kem2 x),
                  if extract_kem2 x <> encap2_public r2 (kem2_pub sk2) 
                     then  dualprf2(hext(const0, decap1 (extract_kem1 x) sk1), decap2 (extract_kem2 x) sk2)
                     else  (if hext(const0,decap1 (extract_kem1 x) sk1) = hext(const0,encap1_shared r1 (kem1_pub sk1)) 
                               then diff(dualprf2(hext(const0, encap1_shared r1 (kem1_pub sk1)), rand1),  rand') 
                               else dualprf2(hext(const0,decap1 (extract_kem1 x) sk1), rand1)))).
...
diff(dualprf(w,if b then u else v),z) = diff(if b then dualprf(w,u) else dualprf(w,v),z).
Proof.
case b => //. 
Qed.

lemma [Game01/right,Game12/left] push_if_above_dualprf_bis  (b:boolean)  (u,v,w,z:message) :
diff(z,dualprf(w,if b then u else v)) = diff(z,if b then dualprf(w,u) else dualprf(w,v)).
Proof.
case b => //. 
Qed.
...
lemma [Game12] push_if_above_dualprf1_simpl (b:boolean) (u,v,w:message):
dualprf2(w,if b then u else v) = if b then dualprf2(w,u) else dualprf2(w,v).
Proof.
case b => //.
Qed.

lemma [Game01/right,Game12/left] simpl_if_diff (u,v,w:message):
diff(dualprf2 (u,v), if u = w then dualprf2(w,v) else dualprf2(u,v)) = dualprf2(u,v).
Proof.
project => //.
case (u =w) => //.
Qed.
...
Proof.
 ...
 + rewrite /frame /transcript /exec /output /cond.  
   fa (_,_,_), !<_,_>.  
   rewrite (ax_dual_prf  (hext(const0,encap1_shared r1 (kem1_pub sk1)), rand1)) in 5. 
   simpl. 
   rewrite /input /state. fa 1. fa(qatt _). {auto. } apply IH. 
 + rewrite /frame /transcript /exec /output /cond. 
   fa (_,_,_), !<_,_>. fa 4. fa 5. 
   rewrite (push_if_above_dualprf (extract_kem2 (input@(Decap(i))) <>
            encap2_public r2 (kem2_pub sk2))  (decap2 (extract_kem2 (input@(Decap(i)))) sk2)  rand1  (hext(const0, decap1 (extract_kem1 (input@(Decap(i)))) sk1))  rand1) in 5.
fa 5. 
    rewrite (ax_dual_prf (hext(const0,(decap1 (extract_kem1 (input@(Decap(i)))) sk1)),
        decap2 (extract_kem2 (input@(Decap(i)))) sk2)) in 5.
    simpl.

   rewrite (ax_dual_prf  (hext(const0, (decap1 (extract_kem1 (input@(Decap(i)))) sk1)), rand1)) in 5.
 simpl.

rewrite (push_if_above_diff (extract_kem2 (input@Decap(i)) <> encap2_public r2 (kem2_pub sk2))
                            (dualprf2(hext (const0, decap1 (extract_kem1 (input@Decap(i))) sk1), decap2 (extract_kem2 (input@Decap(i))) sk2)) 
           (dualprf2(hext (const0, decap1 (extract_kem1 (input@Decap(i))) sk1), rand1))
 (if (hext (const0, decap1 (extract_kem1 (input@Decap(i))) sk1) = hext (const0, encap1_shared r1 (kem1_pub sk1))) then
       (dualprf2(hext (const0, encap1_shared r1 (kem1_pub sk1)), rand1))
     else
       (dualprf2(hext (const0, decap1 (extract_kem1 (input@Decap(i))) sk1), rand1))) ) in 5.

rewrite (simpl_if_diff (hext
          (const0,
           decap1 (extract_kem1 (input@(Decap(i)))) sk1))
        rand1 (hext (const0, encap1_shared r1 (kem1_pub sk1)))) in 5.  

rewrite /input /state. fa 1.  fa(qatt _). {auto. }. apply IH.

 + rewrite /frame /transcript /exec /output /cond.  
   fa (_,_,_), !<_,_>. fa 4. rewrite /input /state. fa 1. fa(qatt _). {auto. } apply IH. 
Qed.
...
: 
  Let moracle = 
    fun x => if x <> hext(const0,encap1_shared r1 (kem1_pub sk1)) then dualprf2(x,rand1) 
  in
  [happens(tau)] -> 
...
Proof.
  ...
  + prf 5 => //.  
    fresh 5 => //. 
  + rewrite /frame /transcript /exec /output. 
    fa(_,_,_); fa !<_,_>. fa 4. rewrite /cond. rewrite /input /state. 
  ...
  rewrite (eq_eq (dualprf2 (hext(const0,decap1 (extract_kem1 (input@Decap(i))) sk1), rand1))
                   (if hext(const0,decap1 (extract_kem1 (input@Decap(i))) sk1) <> 
                       hext(const0,encap1_shared r1 (kem1_pub sk1)) then
                    dualprf2 (hext(const0,decap1 (extract_kem1 (input@Decap(i))) sk1), rand1))). {
rewrite if_true => //. 
    }.
    fa 5. 
    have -> :
      if (hext(const0,decap1 (extract_kem1 (input@Decap(i))) sk1) <>
         ...
         dualprf2 (hext(const0, decap1 (extract_kem1 (input@Decap(i))) sk1), rand1) 
      =
      (moracle (hext(const0, (decap1 (extract_kem1 (input@Decap(i))) sk1)))). by auto.
...
use push_if_above_h_simpl with 
  (extract_kem2 (input@Decap(i)) <> encap2_public r2 (kem2_pub sk2)),  
   dualprf2
...
if (hext (const0, decap1 (extract_kem1 (input@Decap(i))) sk1) 
      ...
      else if (hext (const0, decap1 (extract_kem1 (input@Decap(i))) sk1) 
               <> hext (const0, encap1_shared r1 (kem1_pub sk1))) then
        dualprf2
          (hext (const0, decap1 (extract_kem1 (input@Decap(i))) sk1), rand1), input@Decap(i).
rewrite H0 in 5. clear H0.
fa 5.
use push_if_above_h_simpl with 
(hext (const0, decap1 (extract_kem1 (input@Decap(i))) sk1) =
          hext (const0, encap1_shared r1 (kem1_pub sk1))),
   (if (input@Decap(i) <>
             make_biencap
               (encap1_public r1 (kem1_pub sk1),
                encap2_public r2 (kem2_pub sk2))) then rand'),
if (hext (const0, decap1 (extract_kem1 (input@Decap(i))) sk1) 
               <> hext (const0, encap1_shared r1 (kem1_pub sk1))) then
        dualprf2
          (hext (const0, decap1 (extract_kem1 (input@Decap(i))) sk1), rand1),
input@Decap(i).
...
(** Game 34 *)
(**  rand' is replaved by  dualprf1(hext(const0,encap1_shared r1 (kem1_pub sk1)),rand1) *)
           ...
           out(c_decap, h(make_biencap(extract_kem1 x, extract_kem2 x),
                if extract_kem2 x <> encap2_public r2 (kem2_pub sk2) 
                   then  dualprf2(hext(const0,decap1 (extract_kem1 x) sk1), decap2 (extract_kem2 x) sk2)
                             ...
                             then   diff(rand', dualprf2(hext(const0,encap1_shared r1 (kem1_pub sk1)),rand1))
                             else dualprf2(hext(const0,decap1 (extract_kem1 x) sk1), 
                                 rand1))))).
...
: 
  ...
  [happens(tau)] -> 
  equiv(frame@tau, sk1,r1, sk2,r2,
        diff(rand',dualprf2(hext(const0,encap1_shared r1 (kem1_pub sk1)),rand1)),
        moracle,rand).
Proof.
intro moracle Hap.
induction tau.
  + prf 5 => //.  
    fresh 5 => //. 
  + rewrite /frame /transcript /exec /output /cond. 
    fa (_,_,_); fa !<_,_>. rewrite /state /input. fa 1. fa(qatt _). {auto. } apply IH. 
  + rewrite /frame /transcript /exec /output /cond. 
    fa (_,_,_); fa !<_,_>. rewrite /state /input. fa 1. fa(qatt _). {auto. } apply IH. 
  (** Decap *)
  + rewrite /frame /transcript /output /exec /cond /state. 
    ...
    apply IH.
  + rewrite /frame /transcript /exec /output /cond. 
    fa (_,_,_); fa !<_,_>. fa 4. rewrite /input /state. fa 1. fa(qatt _). {auto. }.
    apply IH.
Qed.
...
(** Game45 *)
(** rand1 is replaced by encap2_shared r2 (kem1_pub sk2) *)
    ...
    && x <> make_biencap(encap1_public r1 (kem1_pub sk1), encap2_public r2 (kem2_pub sk2)) 
       then (
           out(c_decap, h(make_biencap(extract_kem1 x, extract_kem2 x),
                if extract_kem2 x <> encap2_public r2 (kem2_pub sk2) 
                   then  dualprf2(hext(const0,decap1 (extract_kem1 x) sk1), decap2 (extract_kem2 x) sk2)
                   else  (if hext(const0, decap1 (extract_kem1 x) sk1) = 
                             hext(const0, encap1_shared r1 (kem1_pub sk1)) 
                             then   dualprf2(hext(const0,encap1_shared r1 (kem1_pub sk1)), 
                 diff(rand1, encap2_shared r2 (kem2_pub sk2)))
                             else dualprf2(hext(const0,decap1 (extract_kem1 x) sk1), 
                                 diff(rand1, encap2_shared r2 (kem2_pub sk2))))))).
...
global theorem [set:Game45;equiv:Game45]
  ...
  [happens(tau)] -> equiv(frame@tau).
Proof.
  intro Hap.  sym. crypto KEM2_CCA_SINGLE => //.
Qed.
...
intro Hap.
 induction tau.
+ auto.
+ rewrite /frame /transcript /exec /output /cond.  fa (_,_,_), !<_,_>. rewrite bikem_pub_spec.  simpl. 
  rewrite /input /state. fa 1. fa(qatt _). {auto. } apply IH.
+ rewrite /frame /transcript /exec /output /cond.  fa (_,_,_), !<_,_>. 
  rewrite bikem_pub_spec biencap_public_spec. simpl. 
...
use ax_dual_prf with (hext(const0,(decap1 (extract_kem1 (input@Decap(i))) sk1)), (decap2 (extract_kem2 (input@Decap(i))) sk2)).
simpl. 

rewrite !(eq_eq (dualprf2(hext(const0, (decap1 (extract_kem1 (input@Decap(i))) sk1)),
...
rewrite !(ax_dual_prf (hext(const0,(decap1 (extract_kem1 (input@Decap(i))) sk1)),encap2_shared r2 (kem2_pub sk2))).  simpl.  auto. 
...
rewrite !(eq_eq (dualprf2
         (hext(const0,encap1_shared r1 (kem1_pub sk1)), encap2_shared r2 (kem2_pub sk2))) 
         (dualprf
         (hext(const0,decap1 (extract_kem1 (input@Decap(i))) sk1),
        decap2 (extract_kem2 (input@Decap(i))) sk2))).
intro *.  simpl. 
rewrite H. simpl. rewrite Meq. 
rewrite (ax_dual_prf (hext (const0, encap1_shared r1 (kem1_pub sk1)),
   encap2_shared r2 (kem2_pub sk2))). 
auto. 

simpl. 

rewrite !(ax_dual_prf (hext(const0,(decap1 (extract_kem1 (input@Decap(i))) sk1)), (decap2 (extract_kem2 (input@Decap(i))) sk2))) in 4.
simpl.
