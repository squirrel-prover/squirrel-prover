...
(** CPA game for KEMs as in, e.g.,
    ...
    `encap1_shared r (kem1_pub skey)` even when the public encapsulation is revealed. *)
...
process P_encap_middle1 =
  out(c_encap,
      <biencap_public (r1,r2) (bikem_pub (sk1,sk2)),
             h(biencap_public (r1,r2) (bikem_pub (sk1,sk2)), dualprf(hext(const0,rand1), encap2_shared r2 (kem2_pub sk2)))>).
...
process P_encap_middle2 =
  out(c_encap,
      <biencap_public (r1,r2) (bikem_pub (sk1,sk2)),
               h(biencap_public (r1,r2) (bikem_pub (sk1,sk2)), dualprf(rand', encap2_shared r2 (kem2_pub sk2)))>).

system [postquantum] middle2 = (Pub: P_pub | Encap: P_encap_middle2).

process P_encap_middle3 =
  out(c_encap,
      <biencap_public (r1,r2) (bikem_pub (sk1,sk2)),
      h(biencap_public (r1,r2) (bikem_pub (sk1,sk2)), rand'')>).

system [postquantum] middle3 = (Pub: P_pub | Encap: P_encap_middle3).


(** -------------------------------------------------------- *)


global theorem [set:real; equiv:real/left,middle1/left] StrongSecrecyPart1(tau:timestamp[const,glob]):
[happens(tau)] ->
  equiv(frame@tau,
        kem1_pub sk1,
        encap1_public r1 (kem1_pub sk1),
        diff(encap1_shared r1 (kem1_pub sk1), rand1),
        (** KEM2 can be broken *)
        r2, sk2).
Proof.
  intro Hap.
  ...
  + rewrite /frame /transcript /exec /cond /output /state.  
    ...
    rewrite bikem_pub_spec /=.  rewrite /input. fa 1. fa(qatt _). {auto. } apply IH.
  (** Test *)
  + rewrite /frame /transcript /exec /cond /output /state. 
    fa (_,_,_), !<_,_>. fa (if _ then _).
...
[happens(tau)] ->
  equiv(frame@tau, 
    (** both KEM can be assume to be broken only the security of hext matters for this step *)
    r1, sk1, r2, sk2).
Proof.
  intro Hap.
  induction tau.
  ...
  (** Pub *)
  + rewrite /frame /transcript /exec /cond /output /state.  
    fa (_,_,_), !<_,_>. rewrite /input. fa 1. fa(qatt _). {auto. } apply IH.
  (** Test *)
  + rewrite /frame /transcript /exec /cond /output. 
    fa (_,_,_), !<_,_>. 
    rewrite  !biencap_public_spec !bikem_pub_spec /=. 
    fa 5. fa 5. fa 5. fa 5. prf 5. fresh 5. auto. 
    rewrite /input. rewrite /state. fa 1. fa(qatt _). {auto. }
    apply IH.
Qed.



global theorem [set:real/left, real/right,  middle2/left, middle3/left; equiv:middle2/left,middle3/left] StrongSecrecyPart3(tau:timestamp[const,glob]):
[happens(tau)] ->
  equiv(frame@tau, 
    (** both KEM can be assume to be broken only the security of dualprf1  matters for this step *)
    r1, sk1, r2, sk2).
Proof.
  intro Hap.
  induction tau.
  ...
  + rewrite /frame /transcript /exec /cond /output. 
    ...
    use ax_dual_prf with (rand', encap2_shared r2 (kem2_pub sk2)). rewrite H in 5. simpl. 
    fa 5. 
    prf 5.     
    fresh 5. auto.  
...
global theorem [set:real; equiv:middle3/left, real/right] StrongSecrecyPart4(tau:timestamp[const,glob]):
[happens(tau)] ->
   equiv(frame@tau,
   (** both KEM can be assume to be broken only the security of h matters for this step *)
   r1, sk1, r2, sk2).
Proof.
  intro Hap.
  induction tau.
...
global theorem [set: real; equiv: real] StrongSecrecy(tau:timestamp[const]):
...
Proof.
  ...
  * trans [middle3/left, middle3/left].  
        - by apply StrongSecrecyPart3.
        - auto.
        - by apply StrongSecrecyPart4.
Qed.
