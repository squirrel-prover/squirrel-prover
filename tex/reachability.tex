...
\begin{figure}[tp]
  ...
  \begin{mathpar}
    ...
    \inferrule[${\wedge}$-L]{
      \Gamma,\phi,\phi'\vdash_{\calp}\psi
    }{
      \Gamma,\phi\wedge\phi'\vdash_{\calp}\psi
    }
    ...
    \inferrule[${\wedge}$-R]{
      ...
      \Gamma \vdash_{\calp} \phi\wedge\phi'
    }
  ...
  \end{center}
  \begin{mathpar}
    \inferrule[${=}$-R]{~}{\Gamma \vdash_{\calp} t=t} \quad\quad
    \inferrule[${=}$-L]{
      (\Gamma\vdash_{\calp}\phi)\{x\mapsto t',x'\mapsto t\}
    }{
      t=t', \Gamma\{x\mapsto t,x'\mapsto t'\} \vdash_{\calp}
      \phi\{x\mapsto t,x'\mapsto t'\}
    }

    \inferrule[$\forall$-L]{
      \Gamma,\phi\{x\mapsto t\} \vdash_{\calp} \psi\\
      \vdash_{\calp} t:\Sort
    }{
      \Gamma,\forall x:\Sort.\phi \vdash_{\calp} \psi}

    \inferrule[$\forall$-R]{
      \Gamma \vdash_{\calp} \phi}{\Gamma \vdash_{\calp} \forall x:\Sort:\phi}
  \end{mathpar}
  % 
  \medskip
  \textbf{Some inference rules for equalities on names.}
  % The last rule only applies when, for any trace model $\TM$,
  % the term $\interp{t}{\TM}{\calp}$ does not contain any message variable and does
  ...
  \begin{mathpar}
    \inferrule{
      ...
      \Gamma,\mathsf{n}[\vec i]=\mathsf{m}[\vec j]\vdash_{\calp} \phi
    }

    \inferrule{
      \Gamma, i_1=j_1, \ldots, i_k=j_k \vdash_{\calp} \phi
    }{
      \Gamma,\mathsf{n}[i_1,\ldots,i_k]=\mathsf{n}[j_1,\ldots,j_k]\vdash_{\calp} \phi
    }
    %
    % \inferrule{~}{
  ...
  \begin{mathpar}
    \inferrule{
      ...
      \Gamma, b[\vec{i}] \leq a[\vec{i}, \vec{j}] \vdash_{\calp} \phi
    }
    \quad\text{ when } a < b
    ...
    \inferrule{
      ...
      \Gamma, a[\vec{i}] = b[\vec{j}] \vdash_{\calp} \phi
    }
    \quad\text{ when } a \ne b
    ...
    \inferrule{
      \Gamma, \tau' = \pre(\tau) \vee \tau' = \tau \vdash_{\calp} \phi
    }{
      \Gamma, \pre(\tau)\leq\tau'\leq\tau \vdash_{\calp} \phi
    }

    \inferrule{
      \Gamma, \pvec{i}=\pvec{j} \vdash_{\calp} \phi
    }{
      \Gamma, a(\pvec{i})=a(\pvec{j}) \vdash_{\calp} \phi
    }

    \inferrule{
      \Gamma, \forall \tau'.~\tau'\leq \tau \Rightarrow \mcond@\tau \vdash_{\calp} \phi
    }{
      \Gamma, \mexec@\tau \vdash_{\calp} \phi
    }
  \end{mathpar}
  % `
  \medskip
  ...
  % $\interp{\Gamma\vdash_{\calp}\phi}{\TM}{\calp}$ does not contain instances of the names
  ...
  % \label{fig:xor-reach}
  \begin{mathpar}
    \inferrule{~}{\Gamma \vdash_{\calp} t\oplus t = 0}

    \inferrule{~}{\Gamma \vdash_{\calp} t\oplus t' = t'\oplus t}

    \inferrule{~}{\Gamma \vdash_{\calp} t\oplus (t'\oplus t'') = (t\oplus t')\oplus t''}

    % \inferrule{
    %   (\Gamma \vdash_{\calp} \phi)\{x\mapsto \mathsf{m}[\vec j]\}
    % }{
    %   (\Gamma \vdash_{\calp} \phi)\{x\mapsto t\oplus\mathsf{n}[\vec i]\}
    % }
  \end{mathpar}
...
\begin{definition}
  ...
  % Let $\calp$ be a protocol, and $C$ be a macros-free many-holed context with one distinguished hole variable $\cdot$ that occurs exactly once in $C$. Let $u$ be a term of the meta-logic with \textbf{no message variables}. For every macros-free term $s$ of the meta-logic, we let $s \tpos^{\calp}_{C} u$ holds if and only if $s \tpos^{\calp}_{C} \interp{u}{\TM}{\calp}$ holds in any $\calp$-trace model $\TM$.
\end{definition}
...
$\ost_{\calp}(u)$ is a set of triples $(t,\vec{i},c)$ such that, for any $\calp$-trace model $\TM$, the interpretation in $\TM$ of the terms of $\ost_{\calp}(u)$ such that $c$ holds in $\TM$ contains all the subterms of~$\interp{u}{\TM}{\calp}$. Moreover, $\vec{i}$ is the set of new (index) variables in $t$ and $c$: $\fv{t,c} \subseteq \fv{u} \cup \{\pvec{i}\}$.
...
\begin{proposition}
  ...
  %     \textstyle
  %     \Gamma,
  %     \bigvee_{h(x,sk) \in \st(t)} m=x  \vdash_{\calp} \phi
  %   }{
  %     \Gamma, t=h(m,sk) \vdash_{\calp} \phi
  %   }
  ...
  %     \vdash_{\calp} \phi
  %   }{
  %     \Gamma, \minp@\tau=h(m,sk) \vdash_{\calp} \phi
  %   }
  %   \tag{macros case}
  % \end{gather*}
\end{proposition}
