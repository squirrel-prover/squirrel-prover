...
\subsection{Macros}
\charlie{In what follows, unless I'm mistaken, the semantics $(\mout@\alpha)^I$ is never given (and similary for all macros actually). I would guess that we are missing the line $ (m[\vec{i}]@\alpha)^I = m^I(\vec{i},\alpha )  $. If this is correct, I don't see why we bother with adding this level, and don't directly give for instance $(\mcond@\tinit)^I = \true$, rather than $\mcond^I(\tinit) = \true$.}
...
\charlie{I believe that the semantics for inputs are wrong. At least, in order to define some interesting notion of equivalence, we need that $    \minp^I(\alpha'_{k+1}) =
    \att_{k}(    \mframe^I(\alpha'_{k}) )$. In other terms, we must not give to the attacker symbols the outputs, if the condition of the action is not satisfied.
    Let us consider the stupid protocol "if false then A: out(k) ||  in(x). if x=k then B: out(diff(ok,ko)".  Intuitively, this is diff equivalent. Indeed A can never be executed, and then with a fresh, x=k is going to be false with overwhelming probability, and B never occurs. However, let us consider the trace A;B. If I interpret
    $\minp@B$ as $\att_k(output@A)$, I get that at this step, the attackers knows $k$ ($k=output@A$), and then, I can build a distinguisher, because the attacker can produce $k$. Thus, with this interpretation, I can construct a model which violates indinstinguishability.
  }
\[\begin{array}{rll}
    \minp^I(\alpha'_{k+1}) &=&
...
\charlie{Linked to my previous comment inside the definition of the protocols, I would have expected actions to only be able to touch states that depends on their indices. Given an action $A(\vec{i})$, We could define its upates as a mapping $S_A$ from $s[\vec{j}]$ to terms for some states $s$ and $\vec{j}$ a subset of $\vec{i}$. Then, the semantics of $s(\vec{i})@\alpha_k$ would be that either $s$ is in the interpreted domain of $S_{\alpha_{k-1}}$, and we thus interpret the corresponding term, or it is identity, and we search recursively.}
