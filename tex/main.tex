...
\begin{definition}
  Given a computational model $\M$, a sequence of actions $A = a_1,\dots, a_n$, its ordered set of implied timestamp is $\{1,\dots,n\}$. Given $\I$ a finite set of indices, $\sigma$ an evaluation of messages for all actions in $A$, and $\phi$ a propositional formula such that all its terms are built over indices in $\I$ and variables bounds by $\sigma$, we denote $A,\I,\M,\sigma \models \phi$ the fact that $A,\I,\M,\sigma$ satisfies $\phi$.
  We first define inductively a transformation of the formulas $\phi^{\I,\A}$ as :
  \begin{itemize}
  \item $ (\tau = \tau')^{\I,A} := \true $ if $\tau \in A$ and $\tau = \tau'$, else $\false$,

  \item $( \tau \leq \tau' )^{\I,A} := \true$ if $\tau,\tau' \in A$ and $\tau \leq \tau'$, else $\false$,

  \item $(\happens(\tau))^{\I,A} := \true$ if $\tau \in A$, else $\false$,
  \item $(i=i')^{\I,A} := \true$ if $i \in I$ and  $i=i'$, else $\false$,

 \item $ (\forall x:\idx.\ \phi) ^{\I,A} :=  \bigwedge_{i\in \I} (\phi\replace{x}{i})^{\I,A}$

  \item $ (\forall x:\timestamp.\ \phi)^{\I,A} := \bigwedge_{1 \leq \tau \leq n} (\phi\replace{x}{\tau})^{\I,A}$

  \end{itemize}

    We then define $\models$ as:
  \begin{itemize}
  \item $A,\I,\M,\sigma \models \forall x_1,\dots,x_n : \mess.\ \phi $ if $\M, \sigma \models \forall x_1,\dots,x_n : \mess. :\phi^{\I,A} \sim \true$,
...
\begin{definition}
  Given a quantified formula $\phi$ and a trace $(A=(a_0,\dots,a_n),\M)$ of $P$, we write $A, \M \models \phi$ when the trace satisfies $\phi$. With $I$ the finite set of indices appearing in $A$ extended with a set of size the number of indices variables in $I$, and $\sigma$ the instantiation of $A$, we define $\models$ as:
  \begin{itemize}
  \item $A,\M \models \phi$ if $A,\M, I,\sigma \models \phi$
  \end{itemize}
