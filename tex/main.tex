...
\begin{definition}
A protocol step $s$ is a tuple $(I,\phi,U,o)$, where $I \subset \I$ is a list of indices variable, $\phi$ is a propositional formula, $U$ is a set of states updates (a mapping state symbols to terms), and $o$ is a list of terms.
All indices appearing in $\phi,U,o$ must appear in $I$.

A protocol is a tree labelled by protocol steps (nodes of arity one), $\|$ (unfixed arity) and $\oplus$ (unfixed arity), along with a mapping from $\Actions$ to the nodes. When going down inside the protocol, the indices set of the steps can only grow.
\end{definition}

Each protocol step may be instantiated an arbitrary number of time using distincts indices. The protocol defines how the protocol steps may be executed. For instance, if some protocol step appears in
the trace with some defined indices, all its ancestors must have been executed previously in the trace.

We may denote a protocol step $s(\ov{i})$ to give explicitely its set of indexes.
For concision, we will denote protocols using $;,\|,\oplus$. For instance, $s_1; (s_2 \| s_3(i))$ represents the protocol where the attacker can first execute $s_1$, and can then execute in any order $s_2$, and an arbitrary number of occurences of $s_3(i)$.
