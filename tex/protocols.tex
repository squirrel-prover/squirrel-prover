...
\begin{definition} \label{def:action} \label{def:proto}
  ...
  \begin{itemize}
    ...
    \item for each memory cell symbol $s \in \States$ of index arity $p$,
      ...
      \charlie{The management of states seems highly complicated. I guess you had some discussions about them, but I think I missed them so I feel a bit lost...
        \begin{itemize}
        \item  How can we have some $\vec{j}$ which are free index variables w.r.t. the indices of the action ? If I understand what follows, inside action $R(i)$, I can update some state $s(j)$, where $i$ and $j$ are interpreted in distinct way by the interpretation $I$. In term of real life implementations, this seems a bit weird.
        \item Why do we ask for an update of each memory cells ? In the rest, we most of the time make them perform an identity.
        \end{itemize}
I guess I can see why having an explicit update for each action makes it easier to interpret states, as we can always define inductively the value of a state at time t w.r.t to the update at time t-1, but I feel like this can be quite confusing for the reader. It might be simpler to give only some updates, and say that it is by default the identity if undefined.
      }
