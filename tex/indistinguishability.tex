...
\begin{figure}[h]
  ...
  \inferrule[FA-DUP]{
    \inferrule{
      ...
      \Gamma \vdash u,
      \mframe @ \pre(\tau),
      \myif \mexec @ \pre(\tau) \mythen \phi_{h} \myelse \bot
    }
  }{
    \Gamma \vdash u,
    \mframe @ \pre(\tau),
    \mexec @ \pre(\tau) \wedge \phi_{h}
  }
  \end{mathpar}
\caption{FA-DUP rule.
  This rule applies only if $\phi_{h}$ is of the form
  $$\forall \tau_0, \tau_0 \leq \tau \Rightarrow \phi(\tau_0,\tau)
  \;\; \text{or} \;\;
  \exists \tau_0, \tau_0 \leq \tau \wedge \phi(\tau_0,\tau)$$
