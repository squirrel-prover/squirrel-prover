...
\begin{itemize}
  ...
  \item $A(\vec k) \in S$ stands for every action in the system (or protocol)
  \item $A(\vec k)^L$ represents the left projection of meta-logic bi-terms and bi-formulas describing the action $A(\vec k)$
  \item $k \sqsubseteq_{\h(\_,\cdot)} u,C$ means that $k$ appears only in key positions in $u,C$
\end{itemize}

\begin{figure}[h]
  \begin{mathpar}
  \inferrule[Fresh]{
    \Gamma \vdash u, \myif \phi_L \wedge \phi_R \mythen 0 \myelse \diff{n_L(\vec {j_L})}{n_R(\vec {j_R})}
  }{
    \Gamma \vdash u, \diff{n_L(\vec {j_L})}{n_R(\vec {j_R})}
  }
  \end{mathpar}
  \begin{mathpar}
    \phi_L = \big( \displaystyle\bigwedge_{n_L(\vec j) \in u^L} j \neq j_L \big)
    \wedge
    \big(
      \displaystyle\bigwedge_{A(\vec k) \in S}
      \forall \vec k \
      \big(\displaystyle\bigvee_{\tau \in u,L} A(\vec k) \leq \tau \big)
      \Rightarrow
      \big(\displaystyle\bigwedge_{n_L(\vec j) \in A(\vec k)^L} j \neq j_L \big)
    \big)
  \end{mathpar}
  \begin{mathpar}
...
\begin{figure}[h]
  ...
  \inferrule[PRF]{
    ...
    \quad \quad
    k \sqsubseteq_{\h(\_,\cdot)} u,C
  }{
    \Gamma \vdash u, C[\h(t,k)]
  }
  \end{mathpar}
  \begin{mathpar}
  \phi_L = \big( \displaystyle\bigwedge_{\h(m,k) \in u^L,C^L} t \neq m \big)
  \wedge
  \big(
    \displaystyle\bigwedge_{A(\vec k) \in S}
    \forall \vec k \
    \big(\displaystyle\bigvee_{\tau \in u^L,C^L} A(\vec k) < \tau \big)
    \Rightarrow
    \big(\displaystyle\bigwedge_{\h(m,k) \in A(\vec k)^L} t \neq m \big)
  \big)
  \end{mathpar}
  \begin{mathpar}
  \phi_R = ... \ \text{(similar, replacing $L$ by $R$)}
