...
\adrien{I am not sure exactly what we want to say in this subsection. I left it as is.}
...
\begin{lemma}
  ...
  \adrien{It is harder to prove the formula above than to prove $\emptyset \vdash_{\calp}  \mframe^L@\tau \sim \mframe^R@\tau$. Maybe there is a typo?}
\end{lemma}
...
\begin{figure}
  \begin{mathpar}
    \inferrule[Expand]{
      \Gamma \vdash_{\calp} t_L \sim t_R
    }{\Gamma \cup \phi \vdash_{\calp} t_L \sim t_R}

    \inferrule[F-Cut]{
      \Gamma \vdash_{\calp} \phi\\      
      \Gamma \cup \phi \vdash_{\calp} t^L \sim t^R
    }{
      \Gamma \vdash_{\calp} t^L \sim t^R
    }

    \inferrule[Sym]{
      \Gamma \vdash_{\calp} t_L \sim t_R
    }{\Gamma \vdash_{\calp} t_R \sim t_L}

    \inferrule[Refl]{~}
    { \Gamma \vdash_{\calp} t \sim t }

    \inferrule[Trans]{
      \Gamma \vdash_{\calp} t_L \sim t_R\\
      \Gamma \vdash_{\calp} t_R \sim t_S
    }{\Gamma \vdash_{\calp} t_L \sim t_S}

    \inferrule[Subst]{
      \Gamma \cup \{t_1 \deq t_2\sim \true \}  \vdash_{\calp} t^L \sim t^R
    }{
      \Gamma \cup \{t_1 \deq t_2\sim \true \} \vdash_{\calp}
      t^L[ t_1 / t_2] \sim  t^R[ t_1 / t_2]
    }

    \inferrule[if-reach]{
      \phi \vdash_{\calp} \false
    }{
      \Gamma  \vdash_{\calp} \myif \phi \mythen t_L \sim  \myif \phi \mythen t_R
    }

    \inferrule[if-equiv]{
      \phi \vdash_{\calp} \psi \Leftrightarrow \psi'
    }{
      \Gamma \vdash_{\calp}
    ...
    \inferrule[${\lnot}$-R]{
      \Gamma \vdash_{\calp} \false \sim \true
    }{
      \Gamma \vdash_{\calp} t^L \sim t^R
    }
    ...
    \inferrule[fresh]{
      ...
      \Gamma \vdash_{\calp} n \sim m
    }

    \inferrule[if-weak]{
      \Gamma \vdash_{\calp} \phi, t_L \sim \phi, t_R
    }{
      \Gamma \vdash_{\calp} \myif  \phi \mythen t_L \sim \myif \phi \mythen t_R
    }

    \inferrule[Dup]{
      \Gamma \vdash_{\calp} t^L,u \sim t^R,v
    }{
      \Gamma \vdash_{\calp} t^L,u,u \sim t^R,v,v
    }
  \end{mathpar}
...
\begin{figure}[h]
  \begin{mathpar}
    \inferrule[Fresh]{
      \Gamma \vdash_{\calp} u, C[\myif \phi_L \wedge \phi_R \mythen 0 \myelse n(\vec {j_0})]
    }{
      \Gamma \vdash_{\calp} u, C[n(\vec {j_0})]
    }
  \end{mathpar}
  \begin{mathpar}
...
\begin{figure}[h]
  \begin{mathpar}
    \inferrule[PRF]{
      \Gamma \vdash_{\calp} u, C[\myif \diff{\phi_L}{\phi_R} \mythen n \myelse \h(t,k(\vec {j_0}))]
      \quad \quad
      k(\_) \sqsubseteq_{\h(\_,\cdot)} u,C,A(\vec i)
    }{
      \Gamma \vdash_{\calp} u, C[\h(t,k(\vec {j_0}))]
    }
  \end{mathpar}
  \begin{mathpar}
...
\begin{figure}[h]
  \begin{mathpar}
    \inferrule[XOR-ind]{
      \Gamma \vdash_{\calp} u, C[\myif \phi_L \wedge \phi_R \mythen m \myelse t \oplus n(\vec {j_0})]
    }{
      \Gamma \vdash_{\calp} u, C[t \oplus n(\vec {j_0})]
    }
  \end{mathpar}
  \begin{mathpar}
