...
\begin{figure}[t]
  \begin{mathpar}
    ...
    \quad \text{ when } \phi \in \hfadup_{\{T\}}
  \end{mathpar}
  ...
  \begin{mathpar}
    \inferrule{
      ...
      (T' \leq T \Rightarrow \phi) \in \hfadup_S
    }
    ...
    \inferrule{
      ...
      (T' \leq T \wedge \phi) \in \hfadup_S
    }

    \inferrule{\phi\in \hfadup_S}{{\cal Q}.\phi \in \hfadup_S}

    \inferrule{t,t' \in \hfadup_S}{(t = t') \in \hfadup_S}
  ...
  \adrien{J'ai renomé $H$ en $\hfadup$, parce que, si je comprends bien, l'intuition c'est que il contient un ensemble de timestamp qu'on sait être éxécutable en hypotèse. Si ce n'est pas le bon nom, ou si il y a mieux, il suffit de changer la macro que j'ai introduit.}
...
\begin{proof}
  ...
  \begin{itemize}
   ...
   \item The other boolean connectives cases (e.g.\ $\vee$), the case $t = t'$ and the case where $\xi$ is a function application are identical to the $\phi_1\wedge\phi_2$ case. \adrien{(j'ai groupé tout les cas qui sont immédiatements $FA + DUP$ comme dans le premier cas)}
   ...
   \item Consider the case of a meta-term $\mout @ T'$ for some $T' \in S$.
     ...
     $\mframe @ T$ using \textsc{Dup}. \adrien{j'ai un peu reformulé parce que je ne comprenais pas ce que tu faisais. C'est juste plein d'égalités et de réécritures en fait. J'espère que c'est mieux comme ça.}
