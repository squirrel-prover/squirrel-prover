...
\begin{proof}
  ...
  \marginpar{\david{Technically it's not an immediate duplicate because
  the exec is in the frame: we change $\mexec$ into
  $\fst(\fst(\mframe))$ using equational reasoning and then use DUP.}}
  ...
  \begin{itemize}
   ...
   \item Consider the case of a meta-term $\mout @ T'$ for some $T' \in S$.
     ...
     such that \david{TODO interpréter par $\TM$ et $\TM'$}
     $$EQ(
