...
As the $\tau$ is not quantified, the previous formula is equivalent to  \[
  ...
  \]
  for any trace model. 
\stefcolor{Note that we can not explicitly add the quantification
  since it is not allowed by our notion of meta-logic equivalence formula.}
...
\begin{definition}
  ...
  The sequent  $\Gamma \vdash_{\calp_1,\calp_2} \pvec{u} \sim \pvec{v}$ is valid \stefcolor{whenever $(\Gamma \Rightarrow \pvec{u} \sim \pvec{v})$ is $(\calp_1,\calp_2)$-valid.}
\end{definition}

\stef{Comme dit plus haut, cette derniere notion ne me semble pas
  avoir ete defini. Quelle terminologie preferez-vous garder ? Le
  valid ou le $S \models u \sim v$ ?}
...
\stef{Ce n'est pas plutot par induction  sur les timestamps en
  considerant l'ordre induit par $\mpred$ ou quelquechose comme ca.
  Quand on parle d'induction sur la longueur de la trace je ne vois
  pas trop de quelle trace on parle en fait.}
\end{proof}
...
\begin{lemma}
  The rules presented in Figure~\ref{fig:lk-ind} are sound, \stef{i.e. the
  conclusion is valid as soon as the premisses are valid.}
\end{lemma}
    ...
    \[\arraycolsep=10pt
      \begin{array}{|c|c|}
        ...
        \inferrule{
          ...
          \Gamma \vdash
          \pvec{u}, C[t \oplus \stefcolor{\mathsf{n}[\pvec{j}]}] \sim \pvec{v}, s
        }
        &
        \inferrule{
...
\stef{dans XOR rule base logic ca doit etre une typo. Exact ? Et
  aussi, c'est peut etre inutile de redefinir le n-fresh pour la regle
  XOR.
Pour le
  reste, j'ai relu et je crois que ca va mais je ne suis pas
  completement a l'aise avec la condition $c$ du happen before et pour
  l'instant je n'ai pas assez d'intuition pour me dire que la
  condition de la meta-logique implique trivialement la condition de
  la base logique meme si je vois bien un joli parallele.}
