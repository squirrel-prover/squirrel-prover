...
module TSContent : OccurrenceContent with type content = Term.term
  ...
  struct
    ...
    let collision_formula 
        ...
        : Term.term 
      =
      let _ = data in (* to bypass the unused variable warning *)
      if not negate then
...
module EmptyContent : OccurrenceContent with type content = unit
  ...
  struct
    ...
    let collision_formula 
          ...
          ~(data : unit) : Term.term =
      let _ = content in (* to prevent the "unused variable" warning *)
      let _ = collision in
      let _ = data in
...
module NameOC : OccurrenceContent with type content = Name.t
  ...
  struct
    ...
    let collision_formula ~(negate : bool)
        ...
        : Term.term =
      let _ = data in (* to avoid the unused variable warning *)
