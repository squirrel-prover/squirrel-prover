...
let is_def ?explain:(explain=false) uf neqs ut =
  ...
  if ut_equal ut uinit then begin
    if explain && dbg then
      pp_dbg "is_def(%a): is equal to %a" pp_ut ut pp_ut uinit;
    true
  end else
  ...
  if is_kpred uf uinit ut then begin
    if explain && dbg then
      pp_dbg "is_def(%a): %a is its k-predecessor" pp_ut ut pp_ut uinit;
    true
  end else
  ...
  List.exists (fun (u,v) ->
      ...
      if explain && b && dbg then
        pp_dbg "is_def(%a): is equal to %a, and %a ≠ %a"
...
let log_cycles sccs =
  ...
  if List.length sccs > 0 then
    pp_dbg "@[<v 2>Adding SCCs equalities:@, %a@]"
  ...
  obind (fun (uf,minj) ->
      obind (fun (uf,maxj) ->
          ...
          if no_case_disj uf u base minj maxj then None
            ...
            if dbg then pp_dbg "@[<v 2>Disjunction:@;\
                                to_split:%a@;\
                                minj:%d@;\
                                maxj:%d@;\
                                base:%a@]"
...
let neq_sat inst g : bool =
  ...
  List.for_all (fun (u,v) ->
      ...
      if violation && dbg then
        pp_dbg "dis-equality %a ≠ %a violated" pp_ut u pp_ut v;
  ...
  for_all (fun u v ->
      ...
      if violation1 && dbg then
        pp_dbg "contradiction: @[<hov>%a ≤ %a@] and@ \
                @[<hov>is_kpred %a %a@]"
          pp_ut u pp_ut v
          pp_ut v pp_ut u;
...
let log_segment_eq (eq : ut * ut) : unit =
  pp_dbg "@[<v 2>Adding segment equality:@, %a@]"
    (Fmt.pair ~sep:(fun ppf () -> Fmt.pf ppf ", ")
       pp_ut pp_ut) eq

let log_split f =
  pp_dbg "@[<v 2>Splitting clause:@, %a@]" Form.pp_disj f
...
let log_new_eqs eqs =
  ...
  pp_dbg "@[<v 2>Adding new equalities:@, %a@]"
    pp_eqs eqs
...
let log_new_neqs neqs =
  ...
  pp_dbg "@[<v 2>Adding new dis-equalities:@, %a@]"
    pp_neqs neqs

let log_done () = pp_dbg "@[<v 2>Model done@]"

let log_instr inst =
  pp_dbg "@[<v 2>Solving:@ %a@]" (pp_constr_instance ~full:false) inst
...
let rec split (instance : constr_instance) : model list =
  try
    if dbg then log_instr instance;
    ...
    begin match find_new_eqs instance g with
      | Some new_eqs ->
        if dbg then log_new_eqs new_eqs;
      ...
      | None -> match find_new_undef instance g with
        | _ :: _ as undefs ->
          ...
          if dbg then log_new_neqs new_neqs;
  ...
  with
  | No_unif ->
    if dbg then pp_dbg "@[<v 2>No_unif@]";
    []

let split_models instance =
  let models = split instance in

  if dbg then pp_dbg "@[<v 1>final models (%d models):@;%a@]"
    (List.length models)
...
let query ~precise (models : models) (terms : Term.terms) =
  if dbg then pp_dbg "%squery: %a"
    (if precise then "precise " else "") (Fmt.list Term.pp) terms;

  let b = query ~precise models terms in
  
  if dbg then pp_dbg "query result: %a : %a" (Fmt.list Term.pp) terms Fmt.bool b;
