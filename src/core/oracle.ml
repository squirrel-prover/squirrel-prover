...
let get_oracle (s:string) table : Term.term option = 
  let ns = Symbols.Oracle.of_lsymb (mk s) table in
...
let add_oracle ((opt_name,opt_val:string*Term.term)) table =
    (* to not overlap other symbol in map *)
    let s = (mk ("oracle_" ^ opt_name)) in
    if Symbols.Oracle.mem_lsymb s table then
      raise Option_already_defined
    else 
      let v = (Oracle_term opt_val) in
      let data = Oracle_data v in
      let table = fst (Symbols.Oracle.declare_exact table s ~data:data
                         (def_of_oracle v)) in
      table
...
let get_oracle_tag_formula h table =
  try match get_oracle ("oracle_" ^ h) table with
  | Some f -> f
  | None -> Term.mk_false
