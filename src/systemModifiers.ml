  ...
  let iterator t =
    ...
    | _ -> t
     ...
     try
       ...
       let new_systemexpr,old_systemexpr = match old_system with
         | Single (Left s as old) -> SE.Left new_system, old
         | Single (Right s as old) -> SE.Right new_system, old
         |  _ -> assert false
       in

       let new_system_e = SystemExpr.pair table old_systemexpr new_systemexpr in
       (* TODO declare axiom -> need to add parameter to declaration *)
       table
