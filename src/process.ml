...
let prepare : process -> process =
  ...
  let update (state:[`Start|`Input]) invars p = match state,p with

    | _, (Apply _ | Let _ | New _ | Alias _) -> state, invars

    | `Start, (Null | Parallel _ | Repl _) -> `Start, invars
    | `Start, In (_,x,_) -> `Input, x::invars
    | `Start, (Exists _ | Set _) -> `Input, "_"::invars
    | `Start, Out _ -> `Start, "_"::invars

    | `Input, Exists _ -> `Input, invars
    | `Input, Set _ -> `Input, invars

    | `Input, Out _ -> `Start, invars
    | `Input, Null -> `Start, invars
  ...
  let prep_state,invars = update prep_state invars p in
  let prep ?(indices=indices) ?(subst=subst) ?(a=a) p =
    prep indices subst invars prep_state a p
  in
  match p with
    ...
    | Repl (i,p) ->
        ...
        Repl (i', prep ~indices:(i'::indices) ~subst p)
    ...
    | Exists (l,f,p,q) ->
        ...
        let p' = prep ~indices:(l'@indices) ~subst:subst' p in
        let q' = prep q in
          Exists (l',f',p',q')
