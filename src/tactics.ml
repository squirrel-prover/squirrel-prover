...
let gamma_or_intro (judge : Judgment.t) sk fk select_pred =
  let sel, nsel =
    Utils.List.split_pred select_pred
      (Gamma.get_facts judge.Judgment.gamma) in
  ...
  let judges =
    ...
    |> List.map (fun fs ->
        Judgment.set_gamma (Gamma.set_facts judge.Judgment.gamma (fs @ nsel)) judge ) in
  sk judges fk
...
let eq_names (judge : Judgment.t) sk fk =
  ...
  let cnstrs = Completion.name_index_cnstrs (Gamma.get_trs judge.Judgment.gamma)
      (Gamma.get_all_terms judge.Judgment.gamma) in
  let judge =
...
let apply proved_goals (gname:string) (subst:subst) (judge : Judgment.t) sk fk =
  let goals = List.filter (fun (name,g) -> name = gname) proved_goals in
    ...
    | [(np,gp)] ->
      ...
      if not( Theta.is_valid judge.Judgment.theta tatom_list) then raise @@ Failure "Constraint on the variables not satisfied.";
