...
(** [subst_macros_ts names ts1 ts2 t] substitutes [ts1] if applied to a macro
  * which name is in [names] to [ts2] in the term [t]. *)
val subst_macros_ts : string list -> Sorts.timestamp term -> Sorts.timestamp term -> 'a term -> 'a term
