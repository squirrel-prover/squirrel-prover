...
let rec not_occurs_mult ?(fv=Sv.empty) (contx : Constr.trace_cntxt)
          (mult : fsymb option) (t : term) (excl : name list) : bool =
  match t with
...
let rec check_pow_g
          ?(fv=Sv.empty) (contx : Constr.trace_cntxt)
          (g : term) (exp : fsymb) (mult:fsymb option) (t : term) (excl : name list)
        : bool
  =
  match t with
  | tt when tt = g -> true
...
(*------------------------------------------------------------------*)
(* allowed_occs describes what occurrences are tolerated in a term t we consider
   NotAllowed: default, we're in the toplevel term, no occurrences allowed directly here
...
(* Used to find occurrences not allowed by CDH or GDH, depending on the "gdh_oracles" boolean *)
(* Finds all possible occurrences of n in t (ground), except
   occurrences appearing in subterms of the form
    ...
    - u = v ^ (something * a * something else) ^ …,
      where no element of excl appear in "…" or "something"  ONLY if oracles is set to true *)
...
let occurrence_formula ?(action:term option=None) (ts : Fresh.ts_occs)
      (env : Vars.env) (n : nsymb) (occ : name_occ) : term =
  ...
  | Some a ->
     ...
     (* don't substitute ts since the variables we renamed should not occur in ts *)
     let phis_time =
       List.map
         (fun (ti:Fresh.ts_occ) -> 
           let (_, vars', sigma') =
           ...
           mk_exists ~simpl:true vars' (mk_leq ~simpl:true renamed_action renamed_ti))
         ts
     in
...
(* Constructs the proof obligation (sequents) for direct or indirect occurrences
   stating that it suffices to prove the goal assuming
   the occurrence occ is equal to n. *)
let occurrence_sequent ?(action:term option=None) (ts:Fresh.ts_occs) (s : sequent) (n : nsymb) (occ : name_occ) : sequent = 
  TS.set_goal (mk_impl ~simpl:false (occurrence_formula ~action:action ts (TS.vars s) n occ) (TS.goal s)) s
...
let has_cdh (g : lsymb) (tbl : Symbols.table) : bool =
  ...
  | _, Symbols.DHgen l -> List.fold_left (fun b x -> b || List.mem x l) false Symbols.([DH_CDH; DH_GDH; DH_DDH])
  | _ -> false
...
(* Finds the parameters of the group (generator, exponentiation, multiplication of exponents), as well as
   t (term), a, and b (names) such that the equality m1 = m2 is t = g^{a**b}.
   Also checks the group has the CDH (if gdh_oracles = false) or GDH (if true) hyp *)
let dh_param (gdh_oracles : bool) (m1:term) (m2:term) (g:lsymb) (tbl:Symbols.table) :
      fsymb * fsymb * fsymb option * term * nsymb * nsymb =
  (* generator *)
  let gen_n = Symbols.Function.of_lsymb g tbl in

  if not gdh_oracles && not (has_cdh g tbl) then
    soft_failure (Tactics.Failure "can only be applied to a DH group generator with CDH assumption");

  if gdh_oracles && not (has_gdh g tbl) then
    soft_failure (Tactics.Failure "can only be applied to a DH group generator with GDH assumption");
...
(* Applies the CDH or GDH hypothesis (depending on gdh_oracles = false/true)
   to hypothesis m in s, if possible, and returns the list of new proof
   obligations (sequents) *) 
let cgdh (gdh_oracles : bool) (m : lsymb) (g : lsymb) (s : sequent) : sequent list =
  let id, hyp = Hyps.by_name m s in
  (* let hyp = TraceLT.expand_all_macros hyp s in *) (* useful? *)
  ...
  try
    ...
    | Some (m1,m2) ->
       let (gen_s, exp_s, mult_s, t, na, nb) = dh_param gdh_oracles m1 m2 g table in
       ...
       let na_dir_occ =
         get_name_except_allowed gdh_oracles cntxt nas excl gen exp_s mult_s t
       in
       let nb_dir_occ =
         get_name_except_allowed gdh_oracles cntxt nbs excl gen exp_s mult_s t
       in
       ...
       let indirect_sequents =
         Iter.fold_macro_support (fun iocc indirect_sequents ->
             ...
             let oa = Some (mk_action iocc.iocc_aname (Action.get_indices iocc.iocc_action)) in
...
let () =
  T.register_general "cdh"
    ~tactic_help:{
      general_help = "Usage: cdh H, g.\nApplies the CDH assumption (including square-CDH) on H using generator g.";
...
let () =
  T.register_general "gdh"
    ~tactic_help:{
      general_help = "Usage: gdh H, g.\nApplies the GDH assumption (including square-GDH) on H with generator g.";
      detailed_help =
        "Local sequent:\n\
         Given a message equality M of the form t=g^{a b}, \
         close the goal if we can  prove that a and b are \
