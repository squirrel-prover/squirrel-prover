...
let jscoq_cmd_of_obj (cobj : < .. > Js.t) =
  (* let array = Js.to_array (coerce cobj) in *)
  (* let cmd = Array.get array 0 in *)
  (* Firebug.console##log ("cmd : " ^ (Yojson.Safe.pretty_to_string cmd)); *)
  let ojson = obj_to_json cobj in
  Firebug.console##log ("cmd in json : " ^ (Yojson.Safe.pretty_to_string ojson));
...
let on_msg msg =
  ...
  (* For debugging *)
  (* Worker.post_message (SquirrelWorker.answer_to_jsobj (Info *)
  (*   (Yojson.Safe.pretty_to_string yo))); *)

  match jscoq_cmd_of_obj msg with
  | Result.Ok cmd  -> SquirrelWorker.execute_cmd cmd
  | Result.Error s -> Firebug.console##log  ("Error in JSON conv: " ^ s ^ " | " ^ (Js.to_string (Json.output msg)))
